...
graph TD
    ...
    subgraph DevServer ["Development Server<br/>(self-hosted runner: myworkspace)"]
        ...
        subgraph Containers ["Dev Docker Containers"]
            Frontend["dev-myworkspace-frontend<br/>React + Vite build<br/>Nginx<br/>host:18080 -> container:80"]
...
| 서비스 | 컨테이너 | 역할 | 포트 |
| --- | --- | --- | --- |
| Frontend | `dev-myworkspace-frontend` | React/Vite 빌드 결과를 Nginx로 서빙하고 내부 API 경로를 proxy | `18080:80` |
...
- Redis, Worker, PostgreSQL 컨테이너는 현재 `docker-compose.dev.yml`에 포함되어 있지 않습니다.
