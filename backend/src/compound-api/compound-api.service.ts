...
export class CompoundApiService {
  ...
  private async revokeSessionIfGroupwareLoggedOut(
    ...
    try {
      ...
      throw new BadGatewayException({
        ...
        upstreamStatus: 401,
        detail: error instanceof Error ? error.message : 'GROUPWARE_LOGIN_CHECK_FAILED',
      });
    }
  ...
  private isCompoundCalculateData(data: unknown): data is CompoundCalculateData {
    ...
    return numberFields.every((field) => typeof value[field] === 'number' && Number.isFinite(value[field]))
      && typeof value.chemical_formula === 'string'
      && Boolean(composition)
      && typeof composition === 'object'
      && Object.values(composition as Record<string, unknown>).every((item) => typeof item === 'string');
  }
