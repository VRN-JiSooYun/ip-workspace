  ...
  async createJobs(userId: string, body: CreateQuantumCalculationDto) {
    ...
    if (!smiles || !compoundDraftKey) {
      throw new BadRequestException(
        "compoundDraftKey and smiles are required.",
      );
    }
    const createdJobs = await this.prisma.client.$transaction(
      body.jobTypes.map((jobType) =>
        this.prisma.client.calculationJob.create({
          data: {
            userId,
            compoundDraftKey,
            jobType,
    ...
    const jobs = await Promise.all(
      createdJobs.map(async (job) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : "3D_PSA_SUBMISSION_FAILED";
          await this.prisma.client.calculationJob.updateMany({
            where: { id: job.id, status: "SUBMITTING" },
            data: {
              status: "FAILED",
    ...
    return {
      jobs: jobs.map((job) => this.serializeJob(job as CalculationJobRecord)),
    };
  }
