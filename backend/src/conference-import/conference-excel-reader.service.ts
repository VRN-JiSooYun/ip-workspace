...
export const parseConferenceExcelSource = (
  ...
  const legacy = /^Exported_(.+)_Abstract_\(\d{4}-\d{2}-\d{2}\)\.xlsx$/i.exec(
    filename,
  );
  if (legacy)
    return {
      sourceFile: filename,
      conferenceKey: legacy[1],
      profile: "LEGACY_EXPORT",
    };

  const metadata =
    /^(.+)_abstract_(detail_all|poster|document|video)\.xlsx$/i.exec(filename);
  if (!metadata) return null;
  return {
    sourceFile: filename,
    conferenceKey: metadata[1],
    profile:
      metadata[2].toUpperCase() === "DETAIL_ALL"
        ? "DETAIL"
  ...
  async inspect(filePath: string): Promise<ConferenceExcelInspection> {
        ...
        issues: [
          {
            ...
            message:
              "Excel filename does not match a supported Conference import profile.",
          },
        ],
      };
    }
    ...
    for await (const worksheet of workbook) {
      ...
      for await (const row of worksheet) {
        ...
        const keyHeader =
          parsed.profile === "LEGACY_EXPORT" ? "id" : "abstract_url";
    ...
    return {
      ...
      headers,
      rowCount,
      skippedParticipationColumns: headers.filter((header) =>
        PARTICIPATION_COLUMNS.has(header),
      ),
      issues,
    };
  }
