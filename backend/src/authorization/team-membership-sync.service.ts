...
export class TeamMembershipSyncService {
  ...
  async ensureForUser(
    ...
    const displayTeamName = rawTeamName
      .normalize("NFKC")
      .trim()
      .replace(/\s+/g, " ");
    ...
    const currentAssignment =
      await this.prisma.client.groupwareTeamAssignment.findUnique({
        where: { userId },
        select: {
          team: {
            select: {
    ...
    if (
      ...
      currentAssignment.team.moduleAccess.length ===
        DEFAULT_TEAM_MODULE_ACCESS.length &&
      currentAssignment.user.organizationMembers.some(
        (member) =>
          member.organizationId === currentAssignment.team.organization.id,
      )
    ) {
      ...
      return await this.prisma.client.$transaction(
        async (tx) => {
          ...
          const previousAssignment =
            await tx.groupwareTeamAssignment.findUnique({
              where: { userId },
              select: { teamId: true },
            });
