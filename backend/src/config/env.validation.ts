...
const ensureEmailDomains = (name: string, value: string | undefined): void => {
...
export const validateEnv = (config: EnvConfig): EnvConfig => {
  ...
  ensureNumber(
    "CONFERENCE_MEDIA_PROBE_SAMPLE_LIMIT",
    config.CONFERENCE_MEDIA_PROBE_SAMPLE_LIMIT,
  );
  ensureNumber(
    "CONFERENCE_IMPORT_POLL_INTERVAL_MS",
    config.CONFERENCE_IMPORT_POLL_INTERVAL_MS,
  );
  ensureNumber(
    "CONFERENCE_IMPORT_MAX_ISSUES_PER_RUN",
    config.CONFERENCE_IMPORT_MAX_ISSUES_PER_RUN,
  );
  ensureNumber(
    "CONFERENCE_IMPORT_CHUNK_SIZE",
    config.CONFERENCE_IMPORT_CHUNK_SIZE,
  );
  ensureNumber(
    "GMAIL_OUTBOX_POLL_INTERVAL_MS",
    config.GMAIL_OUTBOX_POLL_INTERVAL_MS,
  );
  ensureNumber(
    "GMAIL_OUTBOX_LEASE_DURATION_MS",
    config.GMAIL_OUTBOX_LEASE_DURATION_MS,
  );
