...
const buildCompoundRangeFilterValue = (
  min?: number,
  max?: number,
): string | undefined => {
...
export class PatentAnalysisService {
  ...
  private readonly favoriteFolderResolveLocks = new Map<
    string,
    Promise<string>
  >();
  ...
  async getMyPatents(query: PatentListQueryDto) {
    ...
    const result = await this.helperClient.call<PatentListResult>({
      ...
      filter_group_conjunction_list: JSON.stringify(
        builtFilters.filterGroupConjunctions,
      ),
    ...
    return {
      items: (result.partial_rows ?? []).map((item) =>
        this.annotateFavorite(item, query.favoriteOnly),
      ),
  ...
  async getFavorites(query: Pick<PatentListQueryDto, "ownerId">) {
    ...
    const publicationNumbers = (ownerFolder?.patents ?? [])
      .map((patent) =>
        String(
          patent.publication_number ?? patent.publicationNumber ?? "",
        ).trim(),
      )
      .filter(Boolean);
  ...
  async searchCompounds(query: CompoundSearchQueryDto) {
    ...
    const compoundItems = this.getCompoundSearchItems(
      compoundSearchResult,
      query.type,
    );
    const compoundTotalCount = this.getCompoundSearchTotalCount(
      compoundSearchResult,
      query.type,
    );
    const patentCountByCompoundId = this.getCompoundSearchPatentCounts(
      compoundSearchResult,
      query.type,
    );
    const compoundRows = compoundItems
      .map((item) =>
        this.toStructureSearchCompoundRow(item, patentCountByCompoundId),
      )
  ...
  async downloadEmbodimentsExcel(
    ...
    const ownerId = this.getOwnerId(query.ownerId);
    const resolvedBioactivityType =
      bioactivityType === "modified_bioactivity"
  ...
  async searchEmbodiments(
    ...
    if (
      ...
      throw new BadRequestException(
        "rankingMin must be less than or equal to rankingMax",
      );
    }
    if (
      ...
      throw new BadRequestException(
        "bioactivity.min must be less than or equal to bioactivity.max",
      );
    }
    if (
      body.bioactivity &&
      ...
      throw new BadRequestException(
        "A bioactivity minimum or maximum value is required",
      );
    }
    ...
    if (body.bioactivity) {
      ...
      const metadata = detail.data?.[0] ?? {};
      const allowedBioactivityKeys =
        body.dataset === "clean"
          ? metadata.modified_bioactivity_list
          : metadata.bioactivity_list;
      const bioactivityKey = assertSafeDynamicFilterKey(
        body.bioactivity.key,
        "bioactivity.key",
      );
      if (
        !Array.isArray(allowedBioactivityKeys) ||
        !allowedBioactivityKeys.some(
          (value) => String(value) === bioactivityKey,
        )
      ) {
        throw new BadRequestException(
          "bioactivity.key is not available for this patent",
        );
      }
    }
    ...
    if (body.scaffoldRanking !== undefined) {
      addFilter(
        "num#scaffold_ranking",
        "%s = %s",
        String(body.scaffoldRanking),
      );
    }
    ...
    if (body.bioactivity) {
      const key = assertSafeDynamicFilterKey(
        body.bioactivity.key,
        "bioactivity.key",
      );
      const column =
        body.dataset === "clean" ? "modified_bioactivity" : "bioactivity";
    ...
    if (body.scaffold?.trim()) {
      addFilter(
        "str#scaffold",
        "%s ilike '%s'",
        escapeFilterValue(body.scaffold.trim()),
      );
    }
    if (body.rGroup) {
      const rGroupKey = assertSafeDynamicFilterKey(
        body.rGroup.key,
        "rGroup.key",
      );
      addFilter(
    ...
    const result = await this.helperClient.call<EmbodimentListResult>({
      ...
      publication_number: publicationNumber,
      owner_id: ownerId,
      filter_dict: JSON.stringify(
        filters.length > 0 ? { preset: filters } : {},
      ),
      ligand_filter_dict: "[]",
      order_dict: "[]",
      filter_group_conjunction_list: "[]",
    ...
    const items =
      body.dataset === "clean"
        ...
        : (result.partial_rows ?? []);
    const totalRows =
      body.dataset === "clean" ? result.modified_total_rows : result.total_rows;
  ...
  private async resolveDefaultFavoriteFolderId(
    ownerId: string,
  ): Promise<string> {
    ...
    const lock = this.resolveDefaultFavoriteFolderIdUnsafe(ownerId).finally(
      () => {
  ...
  private async resolveDefaultFavoriteFolderIdUnsafe(
    ownerId: string,
  ): Promise<string> {
    ...
    const folders = await this.getFolderTrees(ownerId);
    const root =
      this.findDefaultWorkspaceRoot(folders.myList, ownerId) ??
      (await this.createFolder(ownerId, "myworkspace", "-1"));
    const rootId = this.getFolderId(root);
    if (!rootId) {
      throw new BadGatewayException(
        "Default favorite root folder id was not found",
      );
    }
  ...
  private annotateFavorite(item: unknown, forceFavorite?: boolean) {
  ...
  private async createFolder(
    ...
    if (!created) {
      throw new BadGatewayException(
        `Created favorite folder "${folderName}" was not found`,
      );
    }
    return created;
  }
  ...
  private toFolderList(value: unknown): PatentFolderNode[] {
    return Array.isArray(value)
      ? value.filter((item): item is PatentFolderNode =>
  ...
  private findDirectFolder(
    folders: PatentFolderNode[],
    folderName: string,
  ): PatentFolderNode | null {
    const normalizedFolderName = this.normalizeFolderName(folderName);
    return (
      folders.find(
        (folder) =>
          this.normalizeFolderName(folder.folder_name) === normalizedFolderName,
      ) ?? null
    );
  }

  private findDefaultWorkspaceRoot(
    folders: PatentFolderNode[],
    ownerId: string,
  ): PatentFolderNode | null {
    const roots = folders.filter(
      (folder) =>
        this.normalizeFolderName(folder.folder_name) === "myworkspace",
    );
    return (
      roots.find((folder) =>
        this.findDirectFolder(folder.children ?? [], ownerId),
      ) ??
      roots[0] ??
      null
    );
  ...
  private findFolderRecursive(
    ...
    for (const folder of folders) {
      if (this.normalizeFolderName(folder.folder_name) === normalizedFolderName)
        return folder;
  ...
  private getOwnerId(ownerId?: string): string {
    return (
      ownerId ?? this.configService.get<string>("patentAnalysis.ownerId", "171")
    );
  }

  private getPdfPath(metadata: Record<string, unknown>): string | null {
    const value =
      metadata.ocr_pdf_path ?? metadata.pdf_path ?? metadata.pdf_url;
  ...
  private getCompoundSearchItems(
  ...
  private mergeCompoundDisplayItems(
    ...
    hitItems.forEach((item) => {
      ...
      const compoundId = this.getStringValue(
        hitSource.compound_id ?? hitSource.id ?? hitRecord?._id,
      );
    ...
    return displayItems.map((item) => {
      ...
      const compoundId = this.getStringValue(
        displayRecord.compound_id ?? displayRecord.id ?? displayRecord._id,
      );
  ...
  private getCompoundSearchTotalCount(
    result: CompoundSearchResult,
    type: string,
  ): number {
  ...
  private async searchElasticCompounds(
    ...
    const rangeFilterValue = buildCompoundRangeFilterValue(
      query.rangeMin,
      query.rangeMax,
    );
    return this.helperClient.call<CompoundSearchResult>({
      ...
      ...(query.rangeField && rangeFilterValue
        ? { [query.rangeField]: rangeFilterValue }
        : {}),
  ...
  private async callPatentInsightApi(
    ...
    const baseUrl = this.configService
      .get<string>("patentAnalysis.insightApiUrl", "http://172.16.1.210:8000")
      .replace(/\/$/, "");
    ...
    try {
      ...
      this.logger.error(
        "[PatentInsight] upstream request failed",
  ...
  private toStructureSearchCompoundRow(
    ...
    const record = asRecord(item);
    const source =
      asRecord(record?._source) ?? asRecord(record?.source) ?? record;
    const compoundId = this.getStringValue(
      source?.compound_id ?? source?.id ?? source?._id ?? record?._id,
    );
