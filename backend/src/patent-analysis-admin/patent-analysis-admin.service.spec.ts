...
describe('PatentAnalysisAdminService notification preferences', () => {
  ...
  const createService = (helperResponses: unknown[]) => {
    ...
    helperResponses.forEach((response) => helperCall.mockResolvedValueOnce(response));
  ...
  it('normalizes the helper alarm and target rows without exposing raw user data', async () => {
    const { service } = createService([{
      ...
      selected_rows: [
        { target_name: 'KRAS', keyword: 'G12C', pending: 'true', email: member.email },
      ],
    }]);

    await expect(service.getNotificationPreferences('user-id')).resolves.toEqual({
      enabled: true,
      availableTargets: [{
        targetName: 'EGFR',
  ...
  it('adds only an active target and returns the refreshed canonical state', async () => {
    ...
    await expect(service.addNotificationTarget('user-id', 'egfr')).resolves.toMatchObject({
      enabled: true,
  ...
  it('rejects a target that is not in the active target list', async () => {
    ...
    await expect(service.addNotificationTarget('user-id', 'KRAS'))
